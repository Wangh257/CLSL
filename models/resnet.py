...
class ResNetV1(nn.Module):
    def __init__(self, name='resnet50'):
        ...
        if name == 'resnet50':
            self.encoder = resnet50(in_channel=4, width=0.5)
        elif name == 'resnet18':
            self.encoder = resnet18(in_channel=4, width=0.5)
        elif name == 'resnet101':
            self.encoder = resnet101(in_channel=4, width=0.5)
        else:
...
class ResNetV2(nn.Module):
    def __init__(self, name='resnet50'):
        ...
        if name == 'resnet50':
            self.encoder = resnet50(in_channel=4, width=1)
        elif name == 'resnet18':
            self.encoder = resnet18(in_channel=4, width=1)
        elif name == 'resnet101':
            self.encoder = resnet101(in_channel=4, width=1)
        else:
            raise NotImplementedError('model {} is not implemented'.format(name))

    def forward(self, x1, x2, layer=6):
        feat_l = self.l_to_ab(l, layer)
        feat_ab = self.ab_to_l(ab, layer)
        return feat_l, feat_ab
...
class ResNetV3(nn.Module):
    def __init__(self, name='resnet50'):
        ...
        if name == 'resnet50':
            self.encoder = resnet50(in_channel=4, width=2)
        elif name == 'resnet18':
            self.encoder = resnet18(in_channel=4, width=2)
        elif name == 'resnet101':
            self.encoder = resnet101(in_channel=4, width=2)
        else:
            raise NotImplementedError('model {} is not implemented'.format(name))
...
class MyResNetsLight(nn.Module):
    def __init__(self, name='resnet50v2', mlp=None):
        super(MyResNetsLight, self).__init__()
        self.mlp = mlp
        if name.endswith('v1'):
            self.encoder = ResNetV1(name[:-2])
        elif name.endswith('v2'):
        ...
        if self.mlp:  # hack: brute-force replacement
            dim_mlp = self.encoder.model_fringes.fc.weight.shape[1]
            self.encoder.model_fringes.fc = nn.Sequential(
                nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), self.encoder.model_fringes.fc
            )   
            self.encoder.model_phase.fc = nn.Sequential(
                nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), self.encoder.model_phase.fc
            )                 

        # self.encoder = nn.DataParallel(self.encoder)

    def forward(self, x_fringe, x_phase, layer=7):

        fea_f = self.encoder.model_fringes(x_fringe, layer)
        fea_p = self.encoder.model_phase(x_phase, layer)

        return fea_f, fea_p


class MyResNetsLight_Singel_Encoder(nn.Module):
    def __init__(self, name='resnet50v2', mlp=None):
        super(MyResNetsLight_Singel_Encoder, self).__init__()
        self.mlp = mlp
        if name.endswith('v1'):
            self.model = ResNetV1(name[:-2])
        elif name.endswith('v2'):
            self.model = ResNetV2(name[:-2])
        elif name.endswith('v3'):
            self.model = ResNetV3(name[:-2])
        else:
            raise NotImplementedError('model not support: {}'.format(name))

        # set three mlp layer for different inputs
        if self.mlp:  # hack: brute-force replacement
            dim_mlp = self.model.encoder.fc.weight.shape[1]
    ...
    def forward(self, x_f, x_fenzi, x_fenmu, layer=6):

        fea_f = self.model.encoder(x_f, layer)
        fea_f = self.fc_f(fea_f)

        fea_fenzi = self.model.encoder(x_fenzi, layer)
        fea_fenzi = self.fc_fenzi(fea_fenzi)

        fea_fenmu = self.model.encoder(x_fenmu, layer)
