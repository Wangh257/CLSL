...
class Feature_Dict(nn.Module):
    ...
    self.register_buffer('memory_fringes', torch.rand(data_size, feature_dim).mul_(2 * stdv).add_(-stdv))
    ...
    def forward(self, fea_f, fea_p, y, idx=None):
        ...
        batchSize = fea_i.size(0)
        data_size = self.memory_l.size(0)
        feature_size = self.memory_l.size(1)
        ...
        weight_fringe = weight_fringe.view(batchSize, K + 1, feature_size)
        ...
        weight_phase = weight_ab.view(batchSize, K + 1, feature_size)
        # weight_phase_norm = weight_phase.norm(dim=2, keepdim=True)
        # weight_phase_normlized = weight_phase.div(weight_phase_norm)
        out_fringe = torch.bmm(weight_phase, fea_f.view(batchSize, inputSize, 1))
